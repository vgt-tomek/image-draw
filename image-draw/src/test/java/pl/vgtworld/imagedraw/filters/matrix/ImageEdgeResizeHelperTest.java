...
public class ImageEdgeResizeHelperTest {
	...
	private static final int IMAGE_WIDTH_HEIGHT = 100;
	
	private static final int ADDITIONAL_EDGE_SIZE = 5;
	
	private static final String IMAGE_PATH = "/image-100-100.jpg";
	...
	public void shouldCopyTopEdgeOfImage() throws IOException {
		...
		assertThat(result.getWidth()).isEqualTo(IMAGE_WIDTH_HEIGHT + 2 * ADDITIONAL_EDGE_SIZE);
		assertThat(result.getHeight()).isEqualTo(IMAGE_WIDTH_HEIGHT + 2 * ADDITIONAL_EDGE_SIZE);
		for (int i = 0; i < IMAGE_WIDTH_HEIGHT + 2 * ADDITIONAL_EDGE_SIZE; ++i) {
	...
	public void shouldCopyBottomEdgeOfImage() throws IOException {
		...
		assertThat(result.getHeight()).isEqualTo(IMAGE_WIDTH_HEIGHT + 2 * ADDITIONAL_EDGE_SIZE);
		for (int i = 0; i < IMAGE_WIDTH_HEIGHT + 2 * ADDITIONAL_EDGE_SIZE; ++i) {
			int firstPixel = result.getRGB(i, IMAGE_WIDTH_HEIGHT + ADDITIONAL_EDGE_SIZE - 1);
			for (int j = IMAGE_WIDTH_HEIGHT + ADDITIONAL_EDGE_SIZE; j < IMAGE_WIDTH_HEIGHT + 2 * ADDITIONAL_EDGE_SIZE; ++j) {
				assertThat(result.getRGB(i, j)).isEqualTo(firstPixel);
			}
		}
	}
	...
	public void shouldCopyLeftEdgeOfImage() throws IOException {
		...
		assertThat(result.getWidth()).isEqualTo(IMAGE_WIDTH_HEIGHT + 2 * ADDITIONAL_EDGE_SIZE);
		assertThat(result.getHeight()).isEqualTo(IMAGE_WIDTH_HEIGHT + 2 * ADDITIONAL_EDGE_SIZE);
		for (int j = 0; j < IMAGE_WIDTH_HEIGHT + 2 * ADDITIONAL_EDGE_SIZE; ++j) {
	...
	public void shouldCopyRightEdgeOfImage() throws IOException {
		...
		assertThat(result.getHeight()).isEqualTo(IMAGE_WIDTH_HEIGHT + 2 * ADDITIONAL_EDGE_SIZE);
		for (int j = 0; j < IMAGE_WIDTH_HEIGHT + 2 * ADDITIONAL_EDGE_SIZE; ++j) {
			int firstPixel = result.getRGB(IMAGE_WIDTH_HEIGHT + ADDITIONAL_EDGE_SIZE - 1, j);
			for (int i = IMAGE_WIDTH_HEIGHT + ADDITIONAL_EDGE_SIZE; i < IMAGE_WIDTH_HEIGHT + 2 * ADDITIONAL_EDGE_SIZE; ++i) {
				assertThat(result.getRGB(i, j)).isEqualTo(firstPixel);
			}
		}
	}
